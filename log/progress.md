...
### 2026-09-18 05:10 复测：备案通过，80/443 的拦截消失；换成解析层的问题

用户在阿里云备完案，从外部复测同一组判据：

- **拦截确实解除**：`Host: alexcn.work` 打 80，9/14 是 `403 Server: Beaver` + "Non-compliance ICP Filing"，
  现在是 `200 Server: nginx/1.24.0 (Ubuntu)` + `{"status":"ok","items":24,"llm_configured":true}`。
  SPA 首页 200、无斜杠 301 → `/PureNavigation/main/`、`/api/software` 200 —— 子路径挂载的三条件全部对外成立。
- **443 仍无监听**：`certbot` 从没跑过（9/15 判定它会因同一个拦截失败，就先停了）。现在可以跑：
  LE 校验用的是权威 NS，且 `--nginx` 插件自己会插 challenge location，不依赖我们那条 `location /`。
- **新拦路在解析层**：`223.5.5.5` 和 `114.114.114.114` 对 `alexcn.work` 返回 **NO ANSWER**，
  而 `8.8.8.8` 与权威 `dns27.hichina.com` 都返回 ECS 的地址；服务器本机按 IP 访问一切正常。
  ⇒ 不是机器、不是 nginx、不是备案拦截，是**国内公共递归还没恢复对这个域名的解析**。
  备案接入侧同步常有数小时到一天的延迟；若审核期间手动暂停过解析，各家递归的负缓存也会陆续过期。
  处置在云解析控制台，代码里没有任何东西可改。

**固化的教训**：「部署完成」和「用户能打开」之间隔着两层平台态 —— 机房边缘按 Host/SNI 拦 80/443 的
**备案**，和各家递归的**解析状态**。只在服务器本机 curl 通过，会把平台态误判成配置错。
验证至少要有四个视角：① 机器本机 ② 公网按 IP 直连 ③ 公网带域名 Host ④ 国内/国外递归各一次。

**顺带纠正一条我自己写错的注释**：conf 里"这台 ECS 没配 IPv6，绑 `[::]:80` 会让 nginx 起不来"的前提
